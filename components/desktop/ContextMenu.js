        ...
        for (let i = 0; i < components.length; i++)
        {
            ...
            if (target)
            {
                ...
                else
                {
                    ...
                    this._menu.init();
                    this._menu.addClass("ContextMenu");
                    let xPos = e.pageX;
                    let yPos = e.pageY;

                    if (yPos + this._menu.realHeight > fx.graphics.screenSize().height)
                        yPos = fx.graphics.screenSize().height - this._menu.realHeight - 5;

                    if (xPos + this._menu.realWidth > fx.graphics.screenSize().width)
                        xPos = fx.graphics.screenSize().width - this._menu.realWidth - 5;

                    this._menu.move(xPos, yPos);
                    break;
                }
            }
